...
const DataBarTable = ({ title, data, columns = ['name', 'revenue'], showMoM = true, icon: Icon = Award, iconGradient = 'from-pink-600 via-red-500 to-orange-500', maxItems = 10 }) => {
    ...
    return (
        <div className="chart-card overflow-hidden">
            <h3 className="text-sm font-bold mb-3 sm:mb-4 flex items-center gap-2">
                ...
                <table className="w-full text-left text-xs sm:text-sm">
                    <thead>
                        <tr className="text-slate-400 text-[10px] font-bold uppercase tracking-wider border-b border-slate-100">
                            <th className="pb-2 sm:pb-2.5 pr-2 sm:pr-3 w-6 sm:w-8">#</th>
                            <th className="pb-2 sm:pb-2.5 pr-2 sm:pr-3">Name</th>
                            <th className="pb-2 sm:pb-2.5 pr-2 sm:pr-3 text-right">Revenue</th>
                            <th className="pb-2 sm:pb-2.5 pr-3 text-right hidden sm:table-cell">Units</th>
                            {showMoM && <th className="pb-2 sm:pb-2.5 pr-3 text-right hidden sm:table-cell">MoM</th>}
                            <th className="pb-2 sm:pb-2.5 w-20 sm:w-28 hidden sm:table-cell"></th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        {items.map((item, idx) => (
                            <tr key={idx} className="hover:bg-pink-50/40 transition-colors">
                                ...
                                <td className="py-1.5 sm:py-2 pr-2 sm:pr-3 font-semibold text-slate-700 text-[11px] sm:text-xs max-w-[120px] sm:max-w-[180px] truncate">
                                    {item.name}
                                    {item.subtitle && <span className="block text-[10px] text-slate-400 font-normal">{item.subtitle}</span>}
                                </td>
                                <td className="py-1.5 sm:py-2 pr-2 sm:pr-3 text-right font-mono text-[11px] sm:text-xs font-bold text-slate-700">
